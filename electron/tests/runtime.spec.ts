...
describe('bundled runtime', () => {
  ...
  it('adds missing desktop tool directories to PATH without duplicating entries', () => {
    const env = ensureDesktopPath({ PATH: '/usr/bin:/bin' }, 'darwin', ['/opt/homebrew/bin', '/missing/bin'])
    expect(env.PATH).toBe('/opt/homebrew/bin:/usr/bin:/bin')

    const unchanged = ensureDesktopPath({ PATH: '/opt/homebrew/bin:/usr/bin' }, 'darwin', ['/opt/homebrew/bin'])
    expect(unchanged.PATH).toBe('/opt/homebrew/bin:/usr/bin')
  })
