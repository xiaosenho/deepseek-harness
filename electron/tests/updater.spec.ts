...
function release(overrides: Record<string, unknown> = {}): Record<string, unknown> {
  return {
    ...
    file_url: `${ARTIFACT_BASE_URL}deepseek-harness-0.2.0-mac-arm64.zip`,
    id: 'a9x8k1m3q2w7e4r',
    is_force: false,
    platform: 'macos',
    version: '0.2.0',
...
describe('Electron PocketBase OTA updater', () => {
  ...
  it.each([
    ...
    ['an invalid checksum', updateInfo({ files: [{ sha512: 'not-a-sha512', url: 'deepseek-harness-0.2.0-mac-arm64.zip' }] })],
    ['a foreign download origin', updateInfo({ files: [{ sha512: SHA512, url: 'https://foreign.example/update.zip' }] })],
  ...
  it.each([
    ['win32', 'windows', 'deepseek-harness-0.2.0-win-x64.exe'],
    ['linux', 'linux', 'deepseek-harness-0.2.0-linux-x64.AppImage'],
  ] as const)('maps %s to the %s PocketBase platform', async (platform, expectedPlatform, artifact) => {
    const fetch = vi.fn().mockResolvedValue(response([release({
      file_url: `${ARTIFACT_BASE_URL}${artifact}`,
      platform: expectedPlatform,
    })]))
    const updater = fakeUpdater({
      isUpdateAvailable: true,
      updateInfo: updateInfo({ files: [{ sha512: SHA512, url: artifact }] }),
    })

    await expect(startOtaUpdate(options({ fetch, updater, platform })))
      .resolves.toEqual({
        status: 'ready',
        version: '0.2.0',
        changelog: 'Fixes and stability improvements',
  ...
  it('contains HTTP, schema, and configured-origin failures without blocking startup', async () => {
    ...
    const invalidResponses = [
      ...
      vi.fn().mockResolvedValue(Response.json({ items: [release({ file_url: 'https://attacker.example/update.zip' })] })),
      vi.fn().mockResolvedValue(Response.json({ items: [release({ file_url: `${ARTIFACT_BASE_URL}deepseek-harness-0.2.0-mac-arm64.dmg` })] })),
