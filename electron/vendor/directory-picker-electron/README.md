...
The **Electron-hosted backend** of the [directory-picker seam](../directory-picker/README.md) registers one `native-browse` capability. It inherits the [browse backend](../directory-picker-browse/README.md), so directory listing and creation use Node's native filesystem APIs inside the web-host child on every platform. `pick(signal)` sends a correlated request over that child's existing parent-process IPC channel; the Electron main process owns the OS dialog and replies with a picked path, cancellation, or failure. The web renderer receives no Electron or Node bridge.
