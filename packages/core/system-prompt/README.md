...
- `ctx.systemPrompt.section(section: PromptSection): () => void` Contribute a section. The layer is the calling context's scope: `agent.ctx` contributes to that agent alone, shadowing a same-named global section there. A `complete: true` section suppresses ordinary sections after the assembly waterfall; explicit user overrides may set `appendAfterComplete: true` to follow it. More than one effective complete section rejects assembly, and one section cannot set both flags. Duplicate names within one layer and non-finite orders throw. Disposed with the calling fiber.
...
- `PromptSection` — `{ name, order, text, complete?, appendAfterComplete? }`. Sections are concatenated in ascending `order`. Order bands: `-100` is the harness identity, `0` the deployment persona, tool guidance uses `100–199`. One effective `complete` section suppresses ordinary sections after cooperative assembly; sections explicitly marked `appendAfterComplete` follow it in order.
...
By default every assembly starts with the harness identity below, then the configured persona and ordered plugin sections after strict variable interpolation. `includeHarnessIdentity: false` omits only that fixed opener. Empty sections disappear; scoped sections and variables can shadow globals for one agent. The `system-prompt/assemble` waterfall determines the delivered prompt and tool schemas unless one effective section declares itself complete; that section suppresses ordinary prompt sections, while explicit `appendAfterComplete` user overrides follow it and the waterfall's contexts, tools, and variables remain. Ordered dynamic contexts are separate from system-prompt sections and become sourced user-role snapshots only when present. `includeRuntimeContext: false` or a scoped suppressor removes all such contexts, including listener additions, without disabling the services that own the underlying policy or state.
