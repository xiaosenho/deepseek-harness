...
- **不弹出 Windows 控制台窗口**：普通 Win32 spawn 与 provider 的同步 `taskkill` helper 都传入 `windowsHide: true`，因此 GUI 宿主在执行命令或清理进程树时不会闪现控制台窗口；已配置的 stdio 行为保持不变。参见 [Windows 隐藏控制台决策](../../../.agents/notes/implemented/bug-fix/2026-08-14-windows-subprocesses-without-console-popups.md)。
