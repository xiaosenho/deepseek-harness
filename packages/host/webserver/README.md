...
The package knows no harness concepts and serves no files: the `/api` HTTP bridge and downlink WebSockets are routes owned by the connection plugin, plugin bundles and the HMR event stream are routes owned by the modules/hmr plugins, and dist serving belongs to the fallback owner. The upgrade handler owns the protocol handshake and connection contents; the webserver only delivers the raw socket and request. `host` accepts only `127.0.0.1` (default posture) and `0.0.0.0` (deliberate network exposure). Electron starts the ordinary `dsh web` HTTP carrier and loads its loopback origin in the desktop window. When remote access is enabled, Electron restarts that composition with an overlay that binds the same server plugin to `0.0.0.0`. This package never prints; the URL line belongs to the shell.
