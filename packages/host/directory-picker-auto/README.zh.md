...
- **仅在启动时判定**——一次判定服务通用 Web 本次启动的所有客户端。同时服务本地窗口与 LAN 浏览器的 Electron 部署使用专用的 [`native-browse` 提供方](../directory-picker-electron/README.md)和感知连接类型的 client 流程，而不使用本选择器；通用选择器仍不提供按连接通告。
