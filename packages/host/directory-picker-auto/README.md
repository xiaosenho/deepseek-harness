...
- **Boot-time only** — one resolution serves every client of a general Web boot. The Electron deployment that serves a local window and LAN browsers at once uses the dedicated [`native-browse` provider](../directory-picker-electron/README.md) and a connection-aware client flow instead of this chooser; the generic chooser still has no per-connection advertisement.
