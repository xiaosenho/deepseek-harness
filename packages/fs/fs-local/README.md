...
The **local-filesystem implementation** of the `ctx.fs` provider contract ([`@deepseek-ai/dsh-fs`](../fs)). Backs the thirteen `FileSystem` primitives with the host filesystem; loading it as a plugin populates `ctx.fs`.
...
- **`writeBytes`** — uses the same private staging, fsync, mode/DACL preservation, per-target lock, optional `FsWriteIntent`, and atomic publication as `writeText`, but stores the supplied bytes without decoding and returns their byte count instead of a contextual text diff.
