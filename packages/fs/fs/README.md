...
| Layer | Package | Role |
|---|---|---|
| tool / executor | `@deepseek-ai/dsh-tool-fs`, specialized consumers such as `@deepseek-ai/dsh-tool-docx` | model-facing schemas and rendering; use `ctx.fs` and dispatch the `fs/*` events |
...
| provider contract | `@deepseek-ai/dsh-fs` (this) | `ctx.fs`: execution-world paths, text/raw-byte IO, and atomic mutation primitives (optional version guard); owns the `fs/*` event vocabulary |
...
| `writeBytes(target, content, expected?, signal?)` | Binary-safe atomic create/replace with the same optional `FsWriteIntent` and no-replace publication rules as `writeText`. Returns the stored byte count instead of a text diff basis. |
...
`ctx.fs` is deliberately close to fsspec-style storage primitives — half a level above byte-level `cat`/`open`. Text methods own UTF-8 decoding and binary rejection; raw-byte methods preserve binary content. The seam owns atomic writes and the literal-edit critical section. It does **not** own line windows, numbered lines, rendered footers, document formats, or observed-state. Observed-state, read-before-edit, and version-guarded writes/edits are policy a plugin (`@deepseek-ai/dsh-fs-observation-policy`) ADDS by supplying the optional guard — not provider behavior — so a sandboxed/remote backend inherits no model-facing observation policy.
...
- **Raw-byte writes are whole-buffer operations** — `writeBytes` does not stream or expose a generic model-facing binary writer; specialized consumers generate a complete, bounded artifact before publication.
