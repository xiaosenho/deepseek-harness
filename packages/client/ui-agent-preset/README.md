...
`agentPreset.read`, `copy`, `openDocument`, and `remove` require Host authority ([`dsh-client-connection`](../connection/README.md)): loopback browsers receive it directly, and trusted remote browsers receive it after presenting the configured `remoteAccessToken`; tokenless remote browsers cannot invoke those methods. A composition names the plugins a session runs, so reading one is reconnaissance, and the rest manage the roster and drive the host desktop. `agentPreset.list` remains ordinary — it carries ids, trust, and the two path-free capability flags, and a LAN client's picker needs it.
