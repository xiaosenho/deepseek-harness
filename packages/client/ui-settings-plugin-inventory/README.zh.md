...
该标签页链接到公开的 [GitHub `dsh-plugin` Topic](https://github.com/topics/dsh-plugin)，并给出桌面端导入顺序：检查插件仓库、退出应用、在已安装 dsh CLI 和 pnpm 的环境中运行 `dsh plugin --profile web add <package-or-git-spec>`，然后重新打开应用并在清单中确认插件。Electron 导航策略会在系统浏览器中打开该目录。UI 本身不安装包；第三方组合包作为受信任 Host 代码运行，不受智能体沙箱保护。
...
- **只读 Loader 视图** —— 本地搜索不会额外引入来源、按来源分组、当前浏览器激活诊断或插件修改控件。发现与导入指引不会检查仓库、验证兼容性，也不会从 renderer 安装包。
