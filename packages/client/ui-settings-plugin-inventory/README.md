...
The tab links to the public [GitHub `dsh-plugin` topic](https://github.com/topics/dsh-plugin) and gives the desktop import sequence: review a plugin repository, quit the app, run `dsh plugin --profile web add <package-or-git-spec>` from an environment with the dsh CLI and pnpm installed, then reopen the app and verify the plugin in the inventory. The Electron navigation policy opens the directory in the operating-system browser. The UI does not install packages itself; third-party bundles execute as trusted Host code outside the agent sandbox.
...
- **Read-only Loader view** — local search does not add provenance, current-browser activation diagnosis, grouping by source, or plugin mutation controls. Discovery and import guidance does not inspect repositories, verify compatibility, or install a package from the renderer.
