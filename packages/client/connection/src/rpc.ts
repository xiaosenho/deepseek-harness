 ...
 * is configured, but never elevates `trustedHosts` by itself. A loopback Host
 * uses `loopbackAccessToken` when configured and never accepts the remote
 * token in its place.
 */
...
export interface ConnectionRpcHandlerOptions {
  /** Host authority and configured Host-specific access token accepted by every endpoint in this channel. */
