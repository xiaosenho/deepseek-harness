...
声明步骤在 `src/onboarding-copy.ts` 中持有完整文案和版本。具备 Host 权威的浏览器会通过既有 settings API 比较并写入 `ui-onboarding.welcomeNoticeVersion`；只有明确点击「继续」才会记录当前版本。loopback 浏览器以及提交已配置 `remoteAccessToken` 的受信任远程浏览器具备该权威。无 token 的远程浏览器只在当前进程中保留确认状态，因此重载后声明会再次出现。
