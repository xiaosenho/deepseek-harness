...
项目所有者还仅授权精确的 `@img/sharp-win32-x64` 包身份，且其 manifest 必须继续声明 `Apache-2.0 AND LGPL-3.0-or-later`。生成器会核验该身份、声明、包版本与 libvips 版本，再渲染所需法务材料的链接。它不会把 LGPL 归类为宽松许可证，也不会授权 `sharp` 或其他平台包。[Electron Sharp LGPL 分发决策](2026-08-14-electron-sharp-lgpl-distribution.md)负责规定随包许可证文本、源码来源和发布义务。
...
Sharp Windows 测试证明：只有精确的平台包身份得到授权，其声明条款仍属非宽松；身份、许可证或 libvips 元数据发生变化时，重新生成前就会失败。Electron Builder 测试还会单独要求打包资源中存在声明文件和完整的 LGPL/GPL 文本。
...
**将 Sharp 的 LGPL 条款视为宽松条款，或复用通用原生包允许列表。** 这会掩盖分发义务，并授权所有者未评审的身份。此处例外只匹配一个包名和一个许可证声明字符串；生成器无法提供的法务文件与源码来源则由专门决策记录负责。
...
Sharp Windows 例外固定在一个包身份与一个许可证声明上。身份或声明发生变化时会默认拒绝；包版本或 libvips 版本发生变化时，生成声明与打包法务材料检查会失败，直至维护者完成评审并更新分发记录。
