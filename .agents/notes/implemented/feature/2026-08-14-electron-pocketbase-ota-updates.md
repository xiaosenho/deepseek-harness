...
The ESM main process loads the CommonJS `electron-updater` package lazily after release selection and reads `autoUpdater` from its default export object. Source runs, unsupported platforms, and checks without a newer release never load it.
...
Installed old-to-new update acceptance remains platform-native release evidence. Unit tests cover PocketBase validation, metadata anchoring, the packaged CommonJS export layout, background policy, force policy, and shutdown ordering, but they do not substitute for a signed, notarized Squirrel.Mac installation test.
