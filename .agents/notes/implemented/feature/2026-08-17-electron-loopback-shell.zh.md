...
Electron 主进程启动打包后的 `dsh web --host 127.0.0.1 --port 0`，等待严格的 loopback 就绪行，再把该 URL 加载到受限的 `BrowserWindow`。Electron 只拥有外壳职责：单实例协调、通过系统浏览器打开外部链接、原生菜单，以及退出前完整关闭进程树。
...
原生菜单保留中文更新操作。更新器选择 PocketBase `app_releases` 中与宿主平台匹配的最新记录，校验可信 HTTPS 制品 URL，再把匹配的 generic release 目录交给 `electron-updater`。制品后缀按平台区分：macOS 为 ZIP，Windows 为 NSIS `.exe`，Linux 为 AppImage。
