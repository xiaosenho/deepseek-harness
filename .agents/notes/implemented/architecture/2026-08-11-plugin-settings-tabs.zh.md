...
分区拥有方贡献 `configurable` 标签页，由它声明既有的嵌套 `settings.plugin.item` 列表。配置卡片原有的命名空间绑定、草稿状态、校验与写入均保持不变。`@deepseek-ai/dsh-client-ui-settings-plugin-inventory` 向 `settings.plugins.tab` 贡献 `all` 标签页；它的 Host Loader 观察器、生成的 Remote 命名空间、DTO 与搜索语义保持不变。清单标签页还链接到公开的 GitHub `dsh-plugin` Topic，并展示既有的 `dsh plugin --profile web add <package-or-git-spec>` 导入路径。它不会从 renderer 安装包：Electron 在系统浏览器中打开目录，用户在终端中显式执行受信任代码安装，再重启应用。已停用的清单条目会在摘要和详情中省略重复的“未挂载”运行状态，已启用条目仍显示其 Cordis 阶段。
...
**直接从清单 renderer 安装选中的仓库。** 否决，因为发现元数据不能证明包的兼容性或可信度，而包安装会用不受智能体沙箱保护的代码修改 Host profile。UI 保持信任决策和命令执行清晰可见。

## 影响

Settings 只有一行“插件”导航，排在“Agent 预设”之前，包含**插件配置**与**插件列表**两个标签页。“Agent 预设”仍是独立分区，因为它编辑每个会话的 agent 组装，而非实时 Host Loader 树。

功能所有权保持明确：`ui-settings-plugins` 拥有“插件”页面与可编辑卡片，`ui-settings-plugin-inventory` 拥有只读清单与发现指引，Host／RPC 路径不变。新的“插件”视图只需注册一个 `settings.plugins.tab` 贡献即可加入。
