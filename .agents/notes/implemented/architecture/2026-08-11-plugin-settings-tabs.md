...
The section owner contributes a `configurable` tab that declares the existing nested `settings.plugin.item` list. Configuration cards keep their namespace bindings, draft state, validation, and writes unchanged. `@deepseek-ai/dsh-client-ui-settings-plugin-inventory` contributes an `all` tab to `settings.plugins.tab`; its Host Loader observer, generated Remote namespace, DTO, and search semantics remain unchanged. The inventory tab also links to the public GitHub `dsh-plugin` topic and presents the existing `dsh plugin --profile web add <package-or-git-spec>` import path. It does not install from the renderer: Electron opens the directory in the operating-system browser, and the user runs the trusted-code installation from a terminal before restarting the app. Disabled inventory entries omit the redundant unmounted runtime state from summaries and details, while enabled entries continue to expose their Cordis phase.
...
**Install a selected repository directly from the inventory renderer.** Rejected because discovery metadata does not establish package compatibility or trust, and package installation mutates the Host profile with code that runs outside the agent sandbox. The UI keeps the trust decision and command execution explicit.
...
Feature ownership remains explicit: `ui-settings-plugins` owns the Plugins page and editable cards, `ui-settings-plugin-inventory` owns the read-only inventory and discovery guidance, and the Host/RPC path does not change. A new Plugins view can join by registering one `settings.plugins.tab` contribution.
