...
| `@deepseek-ai/dsh-tool-docx` | `export_docx` | `ctx.tools`、`ctx.fs`、`ctx.systemPrompt`、`ctx.sandboxPolicy under a confining filesystem provider` | `tool/call`、`fs/write-intent`、`fs/observed after successful binary publication`、`tool/result` | - | 结构化简历生成器，通过文件系统 seam 发布有界的 OOXML 归档；它不解析现有 DOCX 或 PDF 文件。 |
...
把最终确定的结构化简历导出为专业的单栏 Microsoft Word `.docx` 文件。
...
来源：[`packages/fs/tool-docx/src/index.ts`](../packages/fs/tool-docx/src/index.ts)

结构化简历生成器，通过文件系统 seam 发布有界的 OOXML 归档；它不解析现有 DOCX 或 PDF 文件。
